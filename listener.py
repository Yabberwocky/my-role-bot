...
class SelfBotListener:
    ...
    def __init__(self, token: str, channel_id: str, queue: asyncio.Queue):
        ...
        self.patterns = {
            'super_craft': re.compile(r"^A Super (.+?) has been crafted by (\S+)!$", re.IGNORECASE),
    ...
    def _classify_message(self, text: str, message_id: str) -> Dict[str, Any]:
        """
        Classifies the given text into one of four categories based on regex patterns.
        Now includes the original message ID in the payload.
        """
        first_letter_match = re.search(r'[a-zA-Z]', text)
        if first_letter_match:
            text = text[first_letter_match.start():]
        else:
            return {'category': 'unclassified', 'text': text, 'original_message_id': message_id}
                ...
                async with websockets.connect(gateway_url) as ws:
                    ...
                    identify_payload = {
                        ...
                        "d": {
                            "token": self.token,
                            "properties": {
                                "$os": "linux", "$browser": "pingslave_listener", "$device": "pingslave_listener"
                            }
                        }
                    }
                        ...
                        if payload['op'] == 0 and payload['t'] == "MESSAGE_CREATE":
                            ...
                            if event_data.get('channel_id') == self.target_channel_id and event_data.get('embeds'):
                                for embed in event_data['embeds']:
                                    ...
                                    if body_text and message_id:
                                        classified_data = self._classify_message(body_text, message_id)
