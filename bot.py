...
async def verify(interaction: discord.Interaction, user: discord.Member):
    ...
    missing_roles = []
    if not role_to_remove: missing_roles.append(f"Unverified Role (ID: {REMOVE_ROLE_ID})")
    if not role_to_add: missing_roles.append(f"Verified Role (ID: {ADD_ROLE_ID_VERIFY})")
    if missing_roles:
    ...
    # Hierarchy check
    bot_member = guild.me
    if bot_member.top_role <= user.top_role or bot_member.top_role <= role_to_add.position or (role_to_remove and bot_member.top_role <= role_to_remove.position):
         await interaction.response.send_message("❌ Hierarchy Error: I cannot manage roles for this user or the roles involved.", ephemeral=True)
    ...
    try:
        # Check if user already has the 'Verified' role and doesn't have 'Unverified'
        has_verified = role_to_add in user.roles
        has_unverified = role_to_remove in user.roles
...
async def unverify(interaction: discord.Interaction, user: discord.Member):
    ...
    # Check if roles exist
    missing_roles = []
    if not role_to_add: missing_roles.append(f"Unverified Role (ID: {REMOVE_ROLE_ID})")
    if not role_to_remove: missing_roles.append(f"Verified Role (ID: {ADD_ROLE_ID_VERIFY})")
    if missing_roles:
        await interaction.response.send_message(f"❌ Setup Error: The following role(s) could not be found: {', '.join(missing_roles)}. Please contact an admin.", ephemeral=True)
    ...
    try:
        ...
        has_verified = role_to_remove in user.roles
...
async def hcverify(interaction: discord.Interaction, user: discord.Member, ingame_name: str):
    ...
    if missing_roles:
        await interaction.followup.send(f"❌ Setup Error: Missing roles: {', '.join(missing_roles)}. Contact admin.", ephemeral=True)
        await log_error(guild, f"HCVerify failed: Missing roles - {', '.join(missing_roles)}", interaction=interaction)
        return

    # Hierarchy checks
    bot_member = guild.me
    if bot_member.top_role <= user.top_role or bot_member.top_role <= role_verified.position or bot_member.top_role <= role_hc.position or (role_unverified and bot_member.top_role <= role_unverified.position):
         await interaction.followup.send("❌ Hierarchy Error: I cannot manage roles for this user or the required roles.", ephemeral=True)
    ...
    if interaction.user.top_role <= user.top_role and interaction.user.id != guild.owner_id:
         ...
         return
    if bot_member.top_role <= user.top_role: # Separate check specifically for nickname
         await interaction.followup.send("❌ Hierarchy Error: I cannot change the nickname for this user.", ephemeral=True)
...
async def unhcverify(interaction: discord.Interaction, user: discord.Member):
    ...
    role_hc = guild.get_role(ADD_ROLE_ID_HC)
    if not role_hc:
        await interaction.followup.send(f"❌ Setup Error: HC Role (ID: {ADD_ROLE_ID_HC}) not found. Contact admin.", ephemeral=True)
        await log_error(guild, f"UnHCVerify failed: HC role not found.", interaction=interaction)
        return

    # Hierarchy checks
    bot_member = guild.me
    if bot_member.top_role <= user.top_role or bot_member.top_role <= role_hc.position:
         await interaction.followup.send("❌ Hierarchy Error: I cannot manage the HC role for this user.", ephemeral=True)
         await log_error(guild, f"UnHCVerify failed: Bot hierarchy too low for {user.mention} or HC role.", interaction=interaction)
         return
    if interaction.user.top_role <= user.top_role and interaction.user.id != guild.owner_id:
         await interaction.followup.send("❌ Hierarchy Error: You cannot manage roles/nicknames for this user.", ephemeral=True)
         return
    # Separate check for nickname hierarchy
    if bot_member.top_role <= user.top_role:
         # We only need to warn if a nickname *needs* resetting
         if user.nick is not None:
             await interaction.followup.send("❌ Hierarchy Error: I cannot reset the nickname for this user.", ephemeral=True)
             await log_error(guild, f"UnHCVerify failed: Bot hierarchy too low to reset nick for {user.mention}.", interaction=interaction)
             # Allow role removal to proceed, but nick reset will fail later if attempted
             # Alternatively, return here if nick reset is critical: return
    ...
    if role_hc not in user.roles:
        await interaction.followup.send(f"ℹ️ {user.mention} does not have the `{role_hc.name}` role.", ephemeral=True)
    ...
    # --- Nickname Reset ---
    # Only attempt reset if the user actually has a nickname
    if user.nick is not None:
        try:
            ...
            # Hierarchy check might have already warned, but log again here
...
async def wither(interaction: discord.Interaction, user: discord.Member, time: app_commands.Range[float, 0.1, 10.0] = 2.0):
    ...
    async def fail_check(log_reason: str, user_message: str, log_error_details: Optional[Exception] = None):
        ...
        # Use followup if already deferred, otherwise use initial response
    ...
    if invoker.id != guild.owner_id and invoker.top_role <= user.top_role:
    ...
    if not original_roles:
        await interaction.followup.send(embed=create_embed(f"ℹ️ {user.display_name} has no roles (besides @everyone) to remove.", discord.Color.orange()))
        return

    try:
        # Phase 1: Remove Roles
        wither_duration_seconds = int(time * 60)
        reason_wither = f"Withered by {invoker.name} ({invoker.id}) for {time:.1f} minutes."

        # Ensure bot still has perms right before editing
        if not bot_member.guild_permissions.manage_roles:
             await fail_check("Bot lost manage_roles perm.", "❌ I seem to have lost permission to manage roles. Aborting.")
             return
        ...
        await interaction.followup.send(embed=create_embed(
            ...
            description=f"{user.mention} has been withered by {invoker.mention} for **{time:.1f} minutes**!\n\n**Roles Removed:** {roles_removed_str}",
            color=discord.Color.dark_purple()
        ))
        await log_info(guild, f"`{user.name}` (`{user.id}`) withered by `{invoker.name}` (`{invoker.id}`) for {time:.1f}m. Roles removed: {', '.join(r.name for r in original_roles)}")
        ...
        try:
            ...
            bot_member_after = guild.me # Re-check bot's state too
            reason_restore = f"Wither ended after {time:.1f} minutes (invoked by {invoker.id})."

            # Check hierarchy again before restoring
            if bot_member_after.top_role <= member_after.top_role:
            ...
            valid_original_roles = [r for r in original_roles if guild.get_role(r.id) is not None]
            if len(valid_original_roles) != len(original_roles):
                 await log_info(guild, f"Wither restore notice: Some original roles for {member_after.name} no longer exist. Restoring valid ones.")
            ...
            await interaction.followup.send(embed=create_embed(f"✨ {member_after.mention}'s roles have been restored!", color=NERDY_YELLOW))
         ...
         await log_error(guild, f"Wither {phase} failed: Bot lacks permissions (Forbidden) for {user.name}.", interaction=interaction)
         # Edit the deferred response if possible
